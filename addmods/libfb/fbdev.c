...
struct fb_base_info _fb_dev_info(const char *filename);
...
{	
	...
	struct fb_base_info info = _fb_dev_info(name);
	tmp = fb_new(info.xres, info.yres, 
				(strncmp(name, "/dev/fb", 7) == 0) ? FB_DEVICE : FB_FILE
			);
	if (tmp == NULL) {
		fclose(fp);
		return NULL;
...
int fb_write_stream(const Fbdev *fbd, void *stream)
{
	FILE *fp = (FILE *) stream;
	if (fbd == NULL || fbd->buf == NULL || fp == NULL)
		return -1;

	fwrite(fbd->buf, sizeof *fbd->buf, fbd->xres * fbd->yres, fp);
	fseek(fp, 0, SEEK_SET);
...
int fb_info(const Fbdev *fbd, void *fptr)
{
	...
	fprintf(fp, "%s:\t%"PRIu32"\n", "xres", fbd->xres);
	fprintf(fp, "%s:\t%"PRIu32"\n", "yres", fbd->yres);
	fprintf(fp, "%s:\t%"PRIu8 " (%s)\n", "device flag", fbd->dev_type,
		(fbd->dev_type == FB_FILE) ? "framebuffer saving" : "framebuffer device");
	return 0;
}
