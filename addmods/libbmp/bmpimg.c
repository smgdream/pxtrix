...
Bmpimg *bmp_read(const char *name)
{
	...
	if (fread(&bmp->bfh.bf_type,		sizeof bmp->bfh.bf_type,		1, fp) !=  1
	 || fread(&bmp->bfh.bf_size,		sizeof bmp->bfh.bf_size,		1, fp) !=  1
	 || fread(&bmp->bfh.bf_reserved1,	sizeof bmp->bfh.bf_reserved1,	1, fp) !=  1
	 || fread(&bmp->bfh.bf_reserved2,	sizeof bmp->bfh.bf_reserved2,	1, fp) !=  1
	 || fread(&bmp->bfh.bf_offbyte,		sizeof bmp->bfh.bf_offbyte,		1, fp) !=  1
	 /* Read bitmap info header */
	 || fread(&bmp->bih, sizeof bmp->bih, 1, fp) != 1) {
		fclose(fp);
		free(bmp);
		return NULL;
	}
	...
	px_ptr_src = px_iobuf;
	px_ptr_dest = bmp->buf;	 
	if (fread(px_iobuf, hgt * bmp_sizeof_line(wid), 1, fp) != 1) {
		fclose(fp);
		bmp_free(bmp);
		return NULL;
	}
...
int bmp_valid(const char *name)
{
	...
	/* Read bitmap info header */
	fseek(fp, BFH_REAL_SIZ, SEEK_SET);
