...
void *draw_the_mdb_line(void *ptr)
{
	...
	for (;line < img->height; line += 64)
		img_for_px(x, y, 0, line, len, 1) {
			int32_t val = in_mandelbrot((double)(x - ((int64_t)img->width >> 1)) * 4.0 / (double)img->width,
										(double)(y - ((int64_t)img->height >> 1)) * 4.0 / (double)img->height);
			uint8_t gray_val = (val < 0) ? 0 : (uint8_t)(pow((double)val / (double)max_it, gamm) * 255);
			*img_px(img, x, y) = clr_lut_magma[gray_val];
		}
